// Lucide (ISC 라이선스) 경로를 인라인으로 옮겨 담았다.
// 아이콘을 추가하려면 여기에 항목을 넣고 profile.ts 에서 이름으로 부른다.
...
/** stroke 기반 라인 아이콘. 색은 currentColor 를 따라간다. */
export function icon(name: IconName, size = 20, strokeWidth = 1.5): string {
  return (
